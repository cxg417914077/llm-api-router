#[tokio::test]
async fn test_health_tracker_per_group() {
    // 测试健康追踪器按组追踪的功能
    ...
    // 初始状态应该是健康的
    assert!(tracker.is_healthy("production", "primary"));
    assert!(tracker.is_healthy("staging", "primary"));

    // 在不同组中记录失败，应该独立追踪
    tracker.record_failure("production", "primary");
    tracker.record_failure("production", "primary");

    // production/primary 仍然健康（2 次失败 < 3 次阈值）
    assert!(tracker.is_healthy("production", "primary"));
    // staging/primary 应该仍然健康（没有失败）
    assert!(tracker.is_healthy("staging", "primary"));

    // 第 3 次失败，production/primary 不健康
    tracker.record_failure("production", "primary");
    assert!(!tracker.is_healthy("production", "primary"));

    // staging/primary 仍然健康
    assert!(tracker.is_healthy("staging", "primary"));

    // 记录成功，恢复健康
    tracker.record_success("production", "primary");
    assert!(tracker.is_healthy("production", "primary"));
}
...
async fn test_provider_key_equality() {
    ...
    // 相同组和方法的 ProviderKey 应该相等
    let key1 = ProviderKey::new("production", "primary");
    let key2 = ProviderKey::new("production", "primary");
    assert_eq!(key1, key2);

    // 不同组的 ProviderKey 应该不相等
    let key3 = ProviderKey::new("staging", "primary");
    assert_ne!(key1, key3);

    // 同组不同 Provider 应该不相等
    let key4 = ProviderKey::new("production", "secondary");
    assert_ne!(key1, key4);
}
